...
# ---------------------------
# DLS (Depth-Limited Search) used by IDDFS
# ---------------------------
...
def iddfs(start, goal, max_depth=50):
    ...
    for depth in range(max_depth + 1):
        path, states_explored = depth_limited_dfs(tuple(start), tuple(goal), depth)
...
solution_path, total_explored = iddfs(initial_state, goal_state, max_depth=40)
end_time = time.time()

if solution_path:
    print(f"IDDFS found a solution in {len(solution_path)-1} moves!")
    print(f"Total states explored: {total_explored}\n")
    for i, step in enumerate(solution_path):
        print(f"Step {i}:")
        print_puzzle(step)
else:
    print(f"IDDFS did NOT find a solution. Total states explored: {total_explored}")
